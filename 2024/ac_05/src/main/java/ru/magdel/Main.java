...
public class Main {
    ...
    static long midPageSum = 0;
    static long fixedMidPageSum = 0;
...
/*        input = input.trim().replace("\r", "")
        ...
        List<Rule> ruleList = Stream.of(rulesArray).map(line -> line.split("\\|"))
                .map(sArray -> new Rule(Integer.parseInt(sArray[0]), Integer.parseInt(sArray[1])))
                .toList();
        var updatesArray = Arrays.asList(updates.split("\n"));
        List<List<Integer>> incorrectUpdates = new ArrayList();
        NextUpdate:
        for (int i = 0; i < updatesArray.size(); i++) {
            var update = updatesArray.get(i);
            var updateList = Arrays.asList(update.trim().split(","))
            ...
            for (int j = 0; j < updateList.size(); j++) {
                for (int k = 0; k < updateList.size(); k++) {
                    ...
                    var value1 = updateList.get(j);
                    var value2 = updateList.get(k);
                    if (j < k && hasDirectRuleViolatons(value1, value2, ruleList)) {
                        incorrectUpdates.add(new ArrayList(updateList));
                        continue NextUpdate;
                    }
                    if (j > k && hasReverseRuleViolatons(value1, value2, ruleList)) {
                        incorrectUpdates.add(new ArrayList(updateList));
                        continue NextUpdate;
                    }
                }
            }
            var midPage = updateList.get(updateList.size() / 2);
            midPageSum += midPage;
        }

        System.out.println(ruleList.size());
        System.out.println(updatesArray.size());
        System.out.println(midPageSum);
        ...
        do {
            prevSum = fixedSum;
            fixOrders2(incorrectUpdates, ruleList);
            fixedSum = midPageSum(incorrectUpdates);
            System.out.println("Iteration: " + prevSum + " -> " + fixedSum + ":"+sameCount);
            sameCount += (fixedSum == prevSum) ? 1 : 0;
            if (fixedSum != prevSum) {
                sameCount = 0;
            }
        } while (fixedSum != prevSum || (sameCount < 4));

        System.out.println("Fixed sum: " + prevSum);


    }

    private static long midPageSum(List<List<Integer>> incorrectUpdates) {
        long fixedMidPageSum = 0;
        for (int i = 0; i < incorrectUpdates.size(); i++) {
            var incorrectUpdate = incorrectUpdates.get(i);
            var midPage = incorrectUpdate.get(incorrectUpdate.size() / 2);
            fixedMidPageSum += midPage;
        }
        return fixedMidPageSum;
    }

    private static void fixOrders(List<List<Integer>> incorrectUpdates, List<Rule> ruleList) {
        for (int i = 0; i < incorrectUpdates.size(); i++) {
            var incorrectUpdate = incorrectUpdates.get(i);
            for (int j = 0; j < incorrectUpdate.size(); j++) {
                IncorrectOrder:
                for (int k = 0; k < incorrectUpdate.size(); k++) {
                    if (j == k) {
                        continue;
                    }

                    var value1 = incorrectUpdate.get(j);
                    var value2 = incorrectUpdate.get(k);
                    if (hasDirectRuleViolatons(value1, value2, ruleList)) {
                        incorrectUpdate.set(j, value2);
                        incorrectUpdate.set(k, value1);
                        //k=0;
                        continue IncorrectOrder;
                    }
                    if (hasReverseRuleViolatons(value1, value2, ruleList)) {
                        incorrectUpdate.set(j, value2);
                        incorrectUpdate.set(k, value1);
                        //k=0;
                        continue IncorrectOrder;
    ...
    private static void fixOrders2(List<List<Integer>> incorrectUpdates, List<Rule> ruleList) {
        for (int i = 0; i < incorrectUpdates.size(); i++) {
            var incorrectUpdate = incorrectUpdates.get(i);
            incorrectUpdate.sort(new Comparator<Integer>() {
                @Override
                public int compare(Integer o1, Integer o2) {
                    boolean directError = hasDirectRuleViolatons(o1, o2, ruleList);
                    boolean reverseError = hasReverseRuleViolatons(o1, o2, ruleList);
                    return directError
                            ? 1
                            : reverseError
                            ? -1
    ...
    private static boolean hasDirectRuleViolatons(int value1, int value2, List<Rule> ruleList) {
        for (int i = 0; i < ruleList.size(); i++) {
            var rule = ruleList.get(i);
            if (rule.before == value2 && rule.after == value1) {
                return true;
            }
        }
        return false;
    ...
    private static boolean hasReverseRuleViolatons(int value1, int value2, List<Rule> ruleList) {
        for (int i = 0; i < ruleList.size(); i++) {
            var rule = ruleList.get(i);
            if (rule.before == value1 && rule.after == value2) {
                return true;
            }
