...
public class Main {
    ...
    private static long solveEquations(List<Equation> equations) {
        long totalCalibrationResult = 0;
        for (Equation equation : equations) {
            if (solveEquation(equation)) {
                totalCalibrationResult += Long.parseLong(equation.getTestValue());
            }
        }
        return totalCalibrationResult;
    }

    private static boolean solveEquation(Equation equation) {
        String[] parts = equation.getParts().split(" ");
        long testValue = Long.parseLong(equation.getTestValue());
        List<Long> numbers = new ArrayList<>();
        for (int i = 0; i < parts.length; i++) {
            numbers.add(Long.parseLong(parts[i]));
        }

        // var equationList = new ArrayList<Integer>();
        return solve(numbers, 0, testValue, 0, 0);
    }

    private static boolean solve(List<Long> numbers, int index, long target, long accumulated, long accumulatedPrev) {
        if (index == numbers.size()) {
            //check equation
            return accumulated == target;
        }

        long accumulatedMul = index == 0 ? numbers.get(index)
                : accumulated * numbers.get(index);
        var solved = solve(numbers, index + 1, target, accumulatedMul, accumulated);
        if (solved) {
            return true;
        }
        long accumulatedSum = accumulated + numbers.get(index);
        solved = solve(numbers, index + 1, target, accumulatedSum, accumulated);
        if (solved) {
            return true;
        }
        if (index == numbers.size()) {
       ...
       /* if (index == numbers.size() - 1) {
        ...
        long num = (long) (numbers.get(index) * Math.pow(10, sNextNum.length()) + numbers.get(index + 1));
        var newNumbers = new ArrayList<Long>(numbers);
        newNumbers.remove(index);
        newNumbers.remove(index);
        newNumbers.add(index, num);
    ...
    private static class Equation {
        ...
        public Equation(String testValue, String parts) {
